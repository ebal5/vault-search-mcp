...
| ツール | 主要引数 | 説明 | 返り値型 |
|---|---|---|---|
| `vault_search` | `query`, `tags?`, `folder?`, `limit?`, `offset?`, `fields?`, `metadata_filter?` | 全文検索（タグ・フォルダ・frontmatter フィルタ付き） | `SearchResponse` |
| `vault_get_note` | `path`, `fields?` | 単一ノートの全文取得 | `NoteDetail`（未存在時 `NoteNotFoundError`） |
| `vault_recent` | `limit?`, `folder?`, `fields?` | 最近更新されたノート一覧 | `list[RecentNote]` |
| `vault_tags` | — | 全タグと出現回数 | `list[TagCount]` |
| `vault_folders` | — | フォルダ構造とノート数 | `list[FolderCount]` |
| `vault_reindex` | `force?` | インデックス再構築 | `ReindexStats` |
| `vault_stats` | — | インデックス統計情報 | `VaultStats` |

### `fields` パラメータ（返却フィールド選択）

`vault_search` / `vault_get_note` / `vault_recent` は `fields: list[str] | None` を受け取る。
指定すると返却モデルのうち列挙したフィールドだけがセットされ、他は未設定（JSON シリアライズ時に除外）となる。
コンテキストウィンドウ節約用途（例: パスだけ欲しい場合）。

```jsonc
// path と title だけ欲しい場合
{"query": "hermes", "fields": ["path", "title"], "limit": 50}
```

- `fields=None` (既定) なら全フィールド返却（通常の Pydantic コンストラクタで厳密検証）
- 空リスト `[]` または未知のフィールド名は `ValidationError`
- 内部的には `model_construct` でバリデーションをバイパスし、指定外フィールドは未セット状態
...
```jsonc
{
  "tools": {
    "vault_search": {
      ...
      "input_schema":  { /* JSON Schema: query, tags, folder, limit, offset, fields, metadata_filter */ },
...
# schema["tools"]["vault_search"]["output_schema"]     -> fields に指定できる名前
...
await session.call_tool("vault_search", {
    ...
    "fields": ["path", "title", "score"],     # 出力スキーマのフィールドから選ぶ
    "limit": 10,
})
```
...
2. 以降の `vault_search` では `fields` で返却フィールドを絞り、`metadata_filter` で構造化条件を AND する
3. 具体的なノート本文が必要になった時だけ `vault_get_note(path)` を叩く（`fields` で絞れる）
...
- **`fields` の返り値**: 指定時は指定外フィールドが**未セット**で返る（`model_construct` 使用でバリデーションをバイパス）。欠落を想定したコードを書く
- **`fields` エラーケース**: 空リスト / 未知フィールド名 / list 以外の型は `ValidationError`。「全件」は `None` で表現する
