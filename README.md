...
| ツール | 説明 | 返り値型 |
|---|---|---|
| `vault_search` | 全文検索（タグ・フォルダフィルタ付き） | `SearchResponse` |
| `vault_get_note` | 単一ノートの全文取得 | `NoteDetail`（未存在時 `NoteNotFoundError`） |
| `vault_recent` | 最近更新されたノート一覧 | `list[RecentNote]` |
| `vault_tags` | 全タグと出現回数 | `list[TagCount]` |
| `vault_folders` | フォルダ構造とノート数 | `list[FolderCount]` |
| `vault_reindex` | インデックス再構築 | `ReindexStats` |
