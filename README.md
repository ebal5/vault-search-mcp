...
### Tool Annotations（MCP auto-approve ヒント）

各ツールには MCP spec の `ToolAnnotations` が付与されており、Claude Desktop などの
クライアントが `readOnlyHint` を参照して **auto-approve** 対象を判定できる。

| ツール | readOnlyHint | destructiveHint | idempotentHint | openWorldHint |
| --- | --- | --- | --- | --- |
| `vault_search` | `true` | — | — | `false` |
| `vault_get_note` | `true` | — | — | `false` |
| `vault_recent` | `true` | — | — | `false` |
| `vault_tags` | `true` | — | — | `false` |
| `vault_folders` | `true` | — | — | `false` |
| `vault_stats` | `true` | — | — | `false` |
| `vault_reindex` | `false` | `false` | `true` | `false` |

`—` は「MCP spec で `readOnlyHint=true` のとき意味を持たないため未設定」を示す。
FastMCP は `None` フィールドを wire 上から落とすため、実際の `tools/list` には現れない。

#### auto-approve との関係

`readOnlyHint=true` のツール（上表の 6 つ）は **vault の `.md` ファイルを一切変更しない**
読み取り専用操作であり、Claude Desktop の `autoApprove` 設定で許可できる。

`vault_reindex` は `readOnlyHint=false` のため auto-approve 対象外となる。
`destructiveHint=false` は「ユーザー向けデータの不可逆損失を引き起こさない」意図で設定している
（派生インデックス DB の再構築のみで、vault 本体の `.md` ファイルは touch しない）。
