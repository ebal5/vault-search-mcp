...
``_OVERVIEW`` / ``_ERRORS[].description`` は ja-JP で固定。field 名
(``version`` / ``step`` / ``tool`` / ``error_code`` 等) のみ英語。多言語対応は
i18n frontmatter (例: ``overview.en`` / ``overview.ja`` 構造) が必要になった
段階で defer する。
...
部 (``overview`` / ``description``) の内容は implementation との drift guard
を意図的に入れていない — test が fragile 化するためで、agent 向け文言は
「単一 SoT の実装記述」ではなく「自然言語ガイド」として保守する前提。

``_RECOMMENDED_FLOW`` は呼出順序の契約 ({step, tool} のみ) に限定し、prose
説明は持たない (#196 Option A)。tool 個別の説明は ``tools[name].description``
を単一 SoT として参照する。
"""
